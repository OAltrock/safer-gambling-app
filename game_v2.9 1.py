...
def track_player_zone(player, current_zone):
    ...
    if new_zone != current_zone:
        if current_zone is not None:
            ...
            entered_zones.append(f"Exited {current_zone} from {direction}")

        if new_zone is not None:
            direction = "top" if player_y < (ZONE_BOUNDARIES[new_zone][0] + ZONE_BOUNDARIES[new_zone][1]) / 2 else "bottom"
            entered_zones.append(f"Entered {new_zone} from {direction}")
...
def reset_game():
    global player, coin_group, hud, control_popup, game_over_metrics_recorded, entered_zones, zone_time_spent, last_zone_update_time
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if game_state == "paused":
            ...
            if resume_button.is_clicked(event):
                game_state = "play"
                # Adjust last_zone_update_time again after clicking resume
                pause_duration = datetime.datetime.now() - pause_start_time
                last_zone_update_time = adjust_for_pause(pause_start_time, last_zone_update_time)
    ...
    if game_state == "start":
        ...
        current_zone = track_player_zone(player, current_zone)
